    ...
    with col_c:
        ...
        if reg == "USA / GLOBAL":
            ...
            if st.button("⚡ START 5 MIN DEMO"):
                ...
                else: st.warning("Please enter company name.")
        else:
            ...
            if st.button("⚡ INICIAR DEMO 5 MIN"):
                if emp: st.session_state.auth = True; st.session_state.demo_mode = True; st.session_state.demo_start = time.time(); st.session_state.emp_final = f"DEMO_{emp}"; st.rerun()
                else: st.warning("Por favor, poné el nombre de tu empresa.")
    st.stop()
...
act = st.text_area("LISTA DE ACTIVOS (FERRARIS, PROPIEDADES, ETC):", key="sc_boss")
if st.button("🧬 SCAN"):
    if act:
...
p_ia = st.text_input("CONSULTA TÉCNICA:", key="ia_boss")
if p_ia: st.info(f"ADVISOR: Analysis for {st.session_state.emp_final} complete. Status: OPTIMAL.")

if st.sidebar.button("🔒 LOGOUT"): st.session_state.auth = False; st.session_state.demo_mode = False; st.rerun()
