...
st.markdown("""
    ...
    .vip-banner { 
        ...
        font-size: 1.5rem; border: 2px solid #ffffff; box-shadow: 0px 0px 20px #d4af37;
    }
    ...
    div.stButton > button { background-color: #1a1a1a; color: #d4af37; border: 2px solid #d4af37; width: 100%; font-weight: bold; height: 3.5em; }
    </style>
...
if not st.session_state.auth and not st.session_state.demo:
    ...
    with c1:
        if st.button("🔑 INGRESAR CON LLAVE VIP"):
            st.session_state.auth = "login_form"; st.rerun()
    with c2:
        if st.button("🚀 PROBAR DEMO GRATUITA (5 MIN)"):
...
if st.session_state.demo and not st.session_state.auth:
    if (time.time() - st.session_state.start_time) > 300:
        ...
        st.markdown("<div class='vip-banner'>ADQUIERA SU LLAVE VIP PARA CONTINUAR</div>", unsafe_allow_html=True)
        if st.button("VOLVER"): st.session_state.demo = False; st.rerun()
        st.stop()
...
if st.session_state.demo: st.warning(f"⚠️ MODO DEMO: {int(300 - (time.time() - st.session_state.start_time))}s restantes")
...
with c1:
    ...
    # ARREGLADO: Números cargados para evitar error
    df_assets = pd.DataFrame({"Activo": ["RE", "Stocks", "Crypto", "Art"], "Valor":})
    st.bar_chart(df_assets.set_index("Activo"))
with c2:
    st.subheader("🤖 IA VIP")
    pregunta = st.text_input("CONSULTA:")
    if pregunta: st.write("🏛️ **IA:** Dylan García, la orden estratégica es MANTENER.")
