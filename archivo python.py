    ...
    with col_c:
        ...
        if st.session_state.pago_step is None:
            if v_sel == "ARGENTINA":
                ...
                if st.button("🔵 PAYPAL"): st.session_state.pago_step = "PP"; st.rerun()
        else:
...
# --- ACÁ VOLVIÓ EL MODO ADMIN (SIDEBAR) ---
st.sidebar.markdown("### 🛡️ PANEL DE SEGURIDAD")
if st.sidebar.checkbox("🔓 ACCESO FUNDADOR (DYLAN)"):
    pin = st.sidebar.text_input("PIN DE COMANDO:", type="password")
    if pin == "DYLAN777":
        ...
        else: st.sidebar.write("Sin visitas nuevas.")
    elif pin != "":
